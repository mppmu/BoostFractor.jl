...
@time result_traceback = @sync @distributed (vcat) for i in 1:length(Deps)
    deps = Deps[i]
    dnu = pi # One could optimize this for maximal boost in 1D before; its left out here for simplicitly and just set to pi
    print("Running dEps = $deps (dNu = $dnu)... \n")
    ...
    #1D (if you calculate in 1D, use (vcat) above instead of (hcat) )
    transformer(sbdry, coords, modes_1D; prop=propagator1D,f=10e9,diskR=diskR)  
end;

full_fields = transformer_trace_back(result_traceback[5], 0, sbdry, coords,modes_1D; prop=propagator1D,f=10e9)
...
@test boost_factor_transformer ≈ [0.9219455134201273, 4.04056703852798, 18.28704972375292, 7.259610230310012, 10.454105269234187, 15.156155900849086, 1.2340396895028822, 1.8976994867509556, 26.47829858914631, 8.193191222497973]
...
@test full_fields ≈ Complex{Float64}[1.4252452348787488 - 0.8148175624088544im 1.42524523487875 - 0.8148175624088595im; -0.7032500773811425 - 1.7882104943840833im -2.135917787152056 + 3.420965357690683im; 1.0999399554091236 - 0.43257371611752315im 2.199879910818247 - 0.8651474322350463im; 0.0 + 0.0im 3.299819866227371 - 1.2977211483525695im]
